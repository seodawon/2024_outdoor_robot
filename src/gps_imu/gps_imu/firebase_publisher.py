...
class FirebasePublisher(Node):
    def __init__(self):
        ...
        # self.timer = self.create_timer(0.5, self.gps_callback) -> no sensor
        # self.timer = self.create_timer(0.7, self.imu_callback) -> no sensor
    ...
    def gps_callback(self, gps_msg):
        self.lat = gps_msg.latitude  # 37.631085838975299 -> no sensor
        self.lon = gps_msg.longitude # 127.05416703835846 -> no sensor
        self.get_logger().info(f"Received GPS data12: latitude={self.lat}, longitude={self.lon}")
                                      
    # IMU 콜백 함수
    def imu_callback(self, imu_msg):
        # Quaternion을 변환하지 않고 yaw 값만 직접 사용
        self.yaw = 60  # 필요에 따라 수정 가능
