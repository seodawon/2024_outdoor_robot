...
public:
	lakibeam1_scan():Node("laser_scan_publisher")
	{
		declare_parameters();
		get_parameters();
		scan_pub = create_publisher<sensor_msgs::msg::LaserScan>(output_topic, 10); // <타입>(토픽, 큐값)
	...
	void scan_publish()
	{
		...
		while (rclcpp::ok())
		{
			...
			if(scan_vec_ready == 1)
			{
				...
				scan.angle_min = DEG2RAD(0 + angle_offset);
				scan.angle_max = DEG2RAD(360 + angle_offset);
